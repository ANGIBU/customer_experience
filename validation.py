...
class ValidationSystem:
            ...
            if train_nums and test_nums:
                ...
                if train_max >= test_min:
                    ...
                    if overlap_ratio > 0.05:
                        issues.append(f"시간 순서 위반: {overlap_ratio:.1%}")
    ...
    def purged_time_series_cv(self, X, y, n_splits=5):
        """갭 적용 시간 기반 교차 검증"""
        print(f"갭 적용 시간 CV (splits={n_splits})")
        ...
        if 'temporal_id' in X.columns:
            ...
            gap_size = int(total_samples * self.temporal_gap)
            fold_size = (total_samples - gap_size * n_splits) // n_splits
            
            if fold_size < 500:
                print(f"폴드 크기 부족: {fold_size}")
                return self.standard_time_series_cv(X, y, n_splits)
    ...
    def standard_time_series_cv(self, X, y, n_splits=5):
        """표준 시간 기반 교차 검증"""
        print(f"표준 시간 CV (splits={n_splits})")
        ...
        print(f"시간 CV 평균: {mean_score:.4f} (+/- {std_score:.4f})")
    ...
    def holdout_validation(self, X_train, y_train, X_val, y_val):
        ...
        if any(data is None for data in [X_train, y_train, X_val, y_val]):
            return {'accuracy': 0.0, 'class_scores': [0.0, 0.0, 0.0]}
    ...
    def stability_test(self, X, y, n_runs=10):
        ...
        stability_score = max(0, 1 - (std_score / mean_score)) if mean_score > 0 else 0
    ...
    def validate_system(self, X_train, y_train):
        ...
        # 종합 점수
        validation_score = (
            holdout_results['accuracy'] * 0.4 +
            purged_results['mean_score'] * 0.4 +
            stability_results['stability_score'] * 0.2
        )
        
        print(f"\n검증 결과:")
        print(f"홀드아웃: {holdout_results['accuracy']:.4f}")
        print(f"갭 CV: {purged_results['mean_score']:.4f}")
        print(f"안정성: {stability_results['stability_score']:.3f}")
        print(f"종합: {validation_score:.4f}")
    ...
    def get_default_results(self):
        """기본 결과"""
        return {
            'holdout': {'accuracy': 0.0, 'class_scores': [0.0, 0.0, 0.0]},
