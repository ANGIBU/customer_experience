...
class FeatureEngineer:
                ...
                if 'support_needs' in train_df.columns:
                    ...
                    if len(safe_data) > 1000:
                        ...
                        if abs(correlation) < 0.1:
                            ...
                            train_processed['after_interaction_lag3'] = train_processed.groupby('ID')['after_interaction'].shift(3)
                            ...
                            if 'after_interaction' in test_df.columns:
                                ...
                                test_processed['after_interaction_lag3'] = test_processed.groupby('ID')['after_interaction'].shift(3)
                        else:
                            # 누수 위험으로 제거
    ...
    def create_business_features(self, df):
        ...
        if all(col in df.columns for col in ['payment_interval', 'contract_length']):
            ...
            # 결제 빈도 카테고리
            df_new['payment_frequency_cat'] = pd.cut(payment_safe, 
                                                   bins=[0, 7, 30, 90, 365, float('inf')], 
                                                   labels=[0, 1, 2, 3, 4])
            df_new['payment_frequency_cat'] = df_new['payment_frequency_cat'].fillna(2)
        ...
        if all(col in df.columns for col in ['frequent', 'tenure']):
            ...
            # 사용 패턴 카테고리
            usage_quartiles = np.percentile(frequent_safe, [25, 50, 75])
            df_new['usage_category'] = np.digitize(frequent_safe, usage_quartiles)
    ...
    def create_statistical_features(self, df):
        ...
        for col in available_cols:
            ...
            # 제곱 변환
            df_new[f'{col}_squared'] = np.power(values, 2)
    ...
    def create_interaction_features(self, df):
        ...
        for feat1, feat2 in key_interactions:
            if feat1 in df.columns and feat2 in df.columns:
                ...
                df_new[f'{feat1}_{feat2}_mult'] = np.clip(df_new[f'{feat1}_{feat2}_mult'], 0, 200000)
                
                # 나눗셈 (분모가 0이 아닐 때)
                val2_safe = np.where(val2 == 0, 1, val2)
                df_new[f'{feat1}_{feat2}_div'] = val1 / val2_safe
                df_new[f'{feat1}_{feat2}_div'] = np.clip(df_new[f'{feat1}_{feat2}_div'], 0, 100)
        ...
        for col in categorical_cols:
            if col in train_df.columns and col in test_df.columns:
                ...
                for train_idx, val_idx in skf.split(train_df, train_df['support_needs']):
                    ...
                    # 스무딩 파라미터 최적화
                    alpha = min(30, max(5, len(fold_train) // 100))
    ...
    def create_clustering_features(self, train_df, test_df):
        ...
        if len(train_df) > 10000:
            ...
            for k in range(4, 10):
                ...
                clusters = kmeans.fit_predict(train_scaled[:5000])  # 샘플링
                score = silhouette_score(train_scaled[:5000], clusters)
    ...
    def select_features_advanced(self, train_df, target_col='support_needs', max_features=80):
        """피처 선택"""
        ...
        mi_selector = SelectKBest(score_func=mutual_info_classif, k=min(max_features * 2, len(variance_features)))
        mi_selector.fit(X_var_df, y)
        mi_features = [variance_features[i] for i, selected in enumerate(mi_selector.get_support()) if selected]
        
        # RFE로 최종 선택
        if len(mi_features) > max_features:
            X_mi_df = train_df[mi_features].fillna(0)
            rf_selector = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
            rfe_selector = RFE(rf_selector, n_features_to_select=max_features, step=5)
            rfe_selector.fit(X_mi_df, y)
            
            selected_features = [mi_features[i] for i, selected in enumerate(rfe_selector.support_) if selected]
        else:
