...
class FeatureEngineer:
    ...
    def handle_after_interaction(self, train_df, test_df, temporal_threshold=None):
        ...
        if 'after_interaction' in train_df.columns:
            print("after_interaction 피처 완전 제거 (데이터 누수 방지)")
            ...
            # 안전한 대체 피처 생성
            self.create_safe_interaction_features(train_processed, test_processed)
        
        return train_processed, test_processed
    
    def create_safe_interaction_features(self, train_df, test_df):
        """안전한 상호작용 피처 생성"""
        safe_features = ['age', 'tenure', 'frequent', 'payment_interval', 'contract_length']
        
        # 기본 상호작용만 생성
        for i, feat1 in enumerate(safe_features):
            if feat1 in train_df.columns and feat1 in test_df.columns:
                for feat2 in safe_features[i+1:]:
                    if feat2 in train_df.columns and feat2 in test_df.columns:
                        # 안전한 비율 계산
                        train_df[f'{feat1}_{feat2}_ratio'] = train_df[feat1] / (train_df[feat2] + 1e-8)
                        test_df[f'{feat1}_{feat2}_ratio'] = test_df[feat1] / (test_df[feat2] + 1e-8)
    ...
    def create_target_encoding(self, train_df, test_df):
        ...
        skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)  # 3-fold로 축소
