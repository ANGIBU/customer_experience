...
class DataPreprocessor:
    ...
    def apply_temporal_filtering(self, train_df, temporal_info=None):
        ...
        if removal_ratio > 0.15:  # 15% 이상 제거되면 더 보수적 기준 적용
    ...
    def handle_missing_values(self, train_df, test_df):
        ...
        if common_numeric:
            ...
            medium_missing_cols = [col for col, ratio in missing_ratios.items() if 0.05 < ratio <= 0.20]
            low_missing_cols = [col for col, ratio in missing_ratios.items() if ratio <= 0.05]
            
            # KNN Imputer (높은 결측치)
            if high_missing_cols:
                if 'knn' not in self.imputers:
                    self.imputers['knn'] = KNNImputer(n_neighbors=3, weights='distance')
    ...
    def select_features_conservative(self, train_df, max_features=55):
        """보수적 피처 선택"""
    ...
    def apply_scaling_conservative(self, train_df, test_df):
        """보수적 스케일링"""
        train_clean = self.safe_data_conversion(train_df)
        test_clean = self.safe_data_conversion(test_df)
    ...
    def validate_data_quality_strict(self, train_df, test_df):
        """엄격한 데이터 품질 검증"""
        issues = []
        ...
        for col in train_numeric_cols:
            if col not in ['ID', 'support_needs']:
                ...
                if len(train_vals) > 0:
                    # 극단적인 값 확인
                    q99_9 = train_vals.quantile(0.999)
                    q0_1 = train_vals.quantile(0.001)
                    extreme_count = ((train_vals > q99_9 * 10) | (train_vals < q0_1 / 10)).sum()
                    if extreme_count > len(train_vals) * 0.001:
                        issues.append(f"{col}_extreme_values: {extreme_count}")
    ...
    def process_data(self, train_df, test_df, temporal_info=None):
        ...
        # 보수적 피처 선택
        if 'support_needs' in train_df.columns:
            selected_features = self.select_features_conservative(train_df, max_features=50)
        ...
        # 보수적 스케일링
        train_df, test_df = self.apply_scaling_conservative(train_df, test_df)
        
        # 엄격한 품질 검증
        quality_ok, issues = self.validate_data_quality_strict(train_df, test_df)
        
        return train_df, test_df
    
    def prepare_data_temporal_optimized(self, train_df, test_df, val_size=0.22, gap_size=0.02):
        """시간 기반 데이터 준비"""
