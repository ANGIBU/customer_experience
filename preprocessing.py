...
class DataPreprocessor:
    ...
    def handle_missing_values(self, train_df, test_df):
        ...
        if common_numeric:
            ...
            # 적응적 대체 전략
            high_missing_cols = [col for col, ratio in missing_ratios.items() if ratio > 0.22]
            medium_missing_cols = [col for col, ratio in missing_ratios.items() if 0.04 < ratio <= 0.22]
            low_missing_cols = [col for col, ratio in missing_ratios.items() if ratio <= 0.04]
            
            # KNN Imputer (높은 결측치)
            if high_missing_cols:
                if 'knn' not in self.imputers:
                    self.imputers['knn'] = KNNImputer(n_neighbors=4, weights='distance')
    ...
    def prepare_data_temporal_optimized(self, train_df, test_df, val_size=0.16, gap_size=0.002):
        """시간 기반 데이터 준비"""
