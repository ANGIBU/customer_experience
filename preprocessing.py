...
class DataPreprocessor:
    ...
    def handle_missing_values(self, train_df, test_df):
        ...
        for col in train_clean.columns:
            if any(keyword in col for keyword in ['customer_value', 'payment_stability', 'usage_intensity', 'support_risk_score']):
    ...
    def select_features_optimized(self, train_df, max_features=60):
        """피처 선택"""
        ...
        high_corr_features = [column for column in upper_tri.columns if any(upper_tri[column] > 0.93)]
    ...
    def apply_scaling(self, train_df, test_df):
        ...
        if 'robust' not in self.scalers:
            ...
            train_robust = self.scalers['robust'].fit_transform(train_clean[common_numeric])
        else:
            train_robust = self.scalers['robust'].transform(train_clean[common_numeric])
        
        test_robust = self.scalers['robust'].transform(test_clean[common_numeric])
        
        if 'quantile' not in self.scalers:
            self.scalers['quantile'] = QuantileTransformer(output_distribution='uniform', random_state=42)
            train_quantile = self.scalers['quantile'].fit_transform(train_clean[common_numeric])
        else:
            train_quantile = self.scalers['quantile'].transform(train_clean[common_numeric])
        
        test_quantile = self.scalers['quantile'].transform(test_clean[common_numeric])
        
        # 가중 결합
        train_scaled = 0.7 * train_robust + 0.3 * train_quantile
        test_scaled = 0.7 * test_robust + 0.3 * test_quantile
    ...
    def process_data(self, train_df, test_df, temporal_info=None):
        ...
        # 피처 선택
        if 'support_needs' in train_df.columns:
            selected_features = self.select_features_optimized(train_df, max_features=60)
    ...
    def prepare_data_temporal_optimized(self, train_df, test_df, val_size=0.18, gap_size=0.04):
        """시간 기반 데이터 준비"""
